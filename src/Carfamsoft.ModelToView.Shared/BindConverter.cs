            ...
            public static Delegate Get(Type type)
            {
                if (!_cache.TryGetValue(type, out var formatter))
                {
                    ...
                    else if (type.IsEnum)
                    {
                        ...
                        var method = _formatEnumValue ?? (_formatEnumValue = typeof(BindConverter).GetMethod(nameof(FormatEnumValueCore), BindingFlags.NonPublic | BindingFlags.Static));
                    ...
                    {
                        // We have to deal invoke this dynamically to work around the type constraint on Enum.TryParse.
                        var method = _formatNullableEnumValue ?? (_formatNullableEnumValue = typeof(BindConverter).GetMethod(nameof(FormatNullableEnumValueCore), BindingFlags.NonPublic | BindingFlags.Static));
